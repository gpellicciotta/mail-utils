...
1. Stored `date` is the raw, client-supplied `Date` header — not
...
2. Allow dumping all emails to the file system location, where each email
   becomes a markdown document with a header section with all meta-data.
   Do this with a new CLI command 'export'

3. Extend command line interface for 'update', 'export' and 'stats' to allow filtering/importing
