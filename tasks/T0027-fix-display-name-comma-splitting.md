---
id: T0027
title: "Fix unquoted comma in a display name splitting one recipient into bogus extra rows"
owner: claude
needs: []
...
- Found via **T0020**'s full-scale (185,742-message) round-trip comparison
  ...
  - A related, separately-confirmed artifact - a bare-name-only `sender` (no email address at all,
    ...
    row[1]` exclusion added in `b6634d6`. Confirmed by reading `local-roundtrip-test.py` directly; not
    part of this task's scope, no further action needed there.
...
- A plain regex substitution (viable for the `@` case, since `@` is a rare, distinctive marker) is
  **not** viable here: a comma is the normal RFC 5322 address-list separator, so a naive "quote anything
  ...
  Needs a real token-merge pass across the whole comma-separated header value.

### Out of Scope

- Any other already-fixed round-trip bug category from T0020's log (header folding, RFC 2047 decoding,
  attachment size, non-UTF-8 text attachments, unquoted `@`) - unaffected by this change.
...
run. T0020 depended on this task's fix for its own final clean round-trip. Branched off **T0020's own
branch** (`task/T0020-full-archive-import-and-eml-roundtrip`), not mainline - the fix extends
`mime_headers.py`, which only existed on T0020's branch at the time, and was validated against
`work-mail`/`scripts/local-roundtrip-test.py`, both also only present there. Merged back into T0020's
branch when done, not directly into `main`.
...
- Verified against real archive data accessible without a full 26 GB re-parse (see Progress & Validation
  Log) - the specific big-archive message named in Scope can only be *definitively* re-confirmed once
  T0020's own final round-trip re-imports `anubex-outlook-backup.pst`, which is that task's own completion
...
- [x] [Read] Root cause confirmed by reading `outlook/messages.py::parse_addresses`/`mime_headers.py`
  directly and cross-checking against the real `data/storage/roundtrip-full.log` output.
- [x] [Implement] `mime_headers.py::quote_unquoted_comma_display_names` implemented: splits `value` on
  top-level commas (respecting already-quoted `"..."` segments), then walks the resulting tokens left to
  right, merging a token into the pending buffer whenever the buffer doesn't yet look like a complete
  recipient (no `<...>` address and no bare `@` in it), flushing the buffer as one recipient once it does -
  exactly the pattern that distinguishes `alice@x.com, Kumar, Rajesh <rajesh.kumar@astadia.com>` (2
  recipients) from `Kumar, Rajesh <rajesh.kumar@astadia.com>` alone (1 recipient). Quotes the display-name
  half of any flushed recipient that contains an unquoted comma before rejoining.
- [x] [Implement] Wired into `outlook/messages.py` and `thunderbird/messages.py` at the existing `@`-fix
  call sites via a small `_quote_display_names` composing helper in each, plus `_format_address` - the
  actual root cause for the Recipient-Table-fallback path, found by correcting the original root-cause
  analysis mid-implementation (see Progress & Validation Log).
- [x] [Verify] Unit tests added: `tests/test_mime_headers.py` (5 new cases for
  `quote_unquoted_comma_display_names`: a single comma-containing recipient, one preceded by a bare
  address, one preceded/followed by a normal `"Name" <addr>` recipient, an already-quoted `"Last, First"
  <addr>` left untouched, and a bare address with no display name at all left as a no-op) and
  `tests/test_pst_integration.py` (a new `_format_address` test using the exact real names/addresses from
  the affected message).
- [x] [Verify] Full test suite: 238 passed, 2 skipped. `ruff check`/`ruff format --check` clean.
- [x] [Verify] Re-ran `import-pst --with-attachments` against the real `personal-email-backup.pst` (279 MB,
  the one smaller archive accessible without a 26 GB re-parse) into a scratch database under the fixed
  code, then `export --format eml` -> `import-eml` -> `scripts/local-roundtrip-test.py compare`: **PASS:
  262 messages compared, no differences found.** No comma-containing display names exist in this
  particular archive, so it doesn't exercise the fixed code path directly, but confirms no regression
  against real data end to end. Definitive confirmation against the actual affected message is deferred to
  T0020's own final round-trip stage (re-imports `anubex-outlook-backup.pst`) - not a gap in this task's
  own completion, since duplicating that ~51-minute, 26 GB re-parse ahead of when T0020 needs to do it
  anyway would be wasted effort.
...
- 2026-09-02: Implemented `mime_headers.py::quote_unquoted_comma_display_names` and wired it into
  ...
  - **Correction to the root-cause analysis in Scope, found while trying to verify the fix against the
    real affected message**: queried the real `work-mail` database directly for
    ...
    immune) rather than the `headers_text`-derived string-splitting path the original fix targeted.
  ...
  - Verified directly: `_format_address("Kumar, Rajesh", "rajesh.kumar@astadia.com")` now returns
    ...
    fragmentation.
  - Added unit tests: `tests/test_mime_headers.py` (5 new cases) and `tests/test_pst_integration.py` (a
    new `_format_address` test using the exact real names/addresses from the affected message). Full
    suite: 238 passed, 2 skipped. `ruff check`/`ruff format --check` clean.
  - Real-data check (not the affected message itself - see caveat below): re-ran `import-pst
    ...
    accessible without a 26 GB re-parse) into a scratch database under this fixed code, then
    `export --format eml` -> `import-eml` -> `scripts/local-roundtrip-test.py compare`:
...
- Definitive proof against the real affected message is deferred to T0020's own final round-trip stage
  (re-imports `anubex-outlook-backup.pst`) - see Progress & Validation Log's caveat; not a gap in this
  task's own completion, since that re-import is T0020's completion criterion, not one this task could
  reach without duplicating a ~51-minute, 26 GB re-parse ahead of when T0020 needs to do it anyway.
